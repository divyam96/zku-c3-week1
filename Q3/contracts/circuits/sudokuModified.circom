...
template sudoku() {
    ...
    component range_proof[9][9][2];


    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            range_proof[i][j][0] = RangeProof(32);
            range_proof[i][j][0].in <==  puzzle[i][j];
            range_proof[i][j][0].range[0] <== 0;
            range_proof[i][j][0].range[1] <== 9;
            assert(range_proof[i][j][0].out == 1); //[assignment] change assert() to use your created RangeProof instead

            range_proof[i][j][1] = RangeProof(32);
            range_proof[i][j][1].in <==  solution[i][j];
            range_proof[i][j][1].range[0] <== 0;
            range_proof[i][j][1].range[1] <== 9;
            assert(range_proof[i][j][1].out == 1); //[assignment] change assert() to use your created RangeProof instead
